...
async def list_my_favorite_ids(
    caller: Identity = Depends(_customer), session: AsyncSession = Depends(get_db)
):
    ...
    """
    return await restaurant_ids(session, caller.user_id)
...
async def list_my_favorites(
    ...
    stmt = (
        ...
        .where(Favorite.user_id == caller.user_id)
...
async def add_favorite(
    ...
    """
    await add(session, caller.user_id, data.restaurant_id)
...
async def remove_favorite(
    ...
    if not await remove(session, caller.user_id, restaurant_id):
